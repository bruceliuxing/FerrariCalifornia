from tensorflow.keras.layers import Input, Conv2DTranspose, BatchNormalization, ReLU, Conv2D, Lambda, Dropout
...
def evaluate_batch_item(batch_item_detections, num_classes, max_objects_per_class=20, max_objects=100,
    ...
    for cls_id in range(num_classes):
        ...
        class_detections = tf.keras.backend.gather(class_detections, nms_keep_indices)
        detections_per_class.append(class_detections)

    batch_item_detections = tf.keras.backend.concatenate(detections_per_class, axis=0)
...
def centernet(num_classes, backbone='hourglass', input_size=384, max_objects=100, score_threshold=0.1,
              nms=False,
              flip_test=False,
              freeze_bn=True):
    assert backbone in ['resnet50', 'resnetv2', 'mobilenetv2', 'resnet101', 'resnet101v2', 'hourglass']
    output_size = input_size // 4
    head_covn = 64
    image_input = Input(shape=(input_size, input_size, 3), name="Input")
    hm_input = Input(shape=(output_size, output_size, num_classes))
    wh_input = Input(shape=(max_objects, 2))
    reg_input = Input(shape=(max_objects, 2))
    reg_mask_input = Input(shape=(max_objects,))
    index_input = Input(shape=(max_objects,))
    #you can design your own encoderNet and decoderNet here
    #encoder
    if backbone == 'hourglass':
        kwargs = {'num_stacks': 1, 'cnv_dim': 256, 'weights': None, 'inres': (None, None), 'dims':[256, 384, 384,384,  512]}
        head_covn = 256
        #heads = {'hm': num_classes, 'reg': 2, 'wh': 2}
        hg_model = HourglassNetwork(input_layer=image_input, **kwargs)
        x = hg_model.outputs[-1]

    else:
        if backbone == 'resnet50':
            resnet = tf.keras.applications.ResNet50(input_tensor=image_input,include_top=False,weights=None)
        elif backbone == 'mobilenetv2':
            resnet = tf.keras.applications.MobileNetV2(input_tensor=image_input, include_top=False, weights=None)

        # (b, 16, 16, 2048)
        C5 = resnet.outputs[-1]

        x = Dropout(rate=0.3)(C5)
        # decoder
        _num_filters = 256
        for i in range(3):
            num_filters = _num_filters // pow(2, i)
    ...
    # hm header
    y1 = Conv2D(num_classes, 1, kernel_initializer='he_normal', kernel_regularizer=l2(5e-4), activation='sigmoid')(x)

    # wh header
    y2 = Conv2D(2, 1, kernel_initializer='he_normal', kernel_regularizer=l2(5e-4))(x)

    # reg header
    y3 = Conv2D(2, 1, kernel_initializer='he_normal', kernel_regularizer=l2(5e-4))(x)

    loss_ = Lambda(loss, name='centernet_loss')([y1, y2, y3, hm_input, wh_input, reg_input, reg_mask_input,index_input])
    tf.compat.v1.summary.scalar(name='centernet_loss', tensor=loss_)
