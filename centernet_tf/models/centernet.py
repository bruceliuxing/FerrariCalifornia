...
def centernet(num_classes, backbone='hourglass', input_size=512, max_objects=100, score_threshold=0.1,
              nms=True,
              flip_test=False,
              freeze_bn=True):
    assert backbone in ['resnet18', 'resnet34', 'resnet50', 'resnet101', 'resnet152', 'hourglass', 'mobilenetv2']
    output_size = input_size // 4
    head_covn = 64
    image_input = Input(shape=(None, None, 3), name="Input")
    ...
    if backbone == 'hourglass':
        kwargs = {'num_stacks': 1, 'cnv_dim': 256, 'weights': None, 'inres': (None, None)}
        head_covn = 256
    ...
    # hm header
    y1 = Conv2D(head_covn, 3, padding='same', use_bias=False, kernel_initializer='he_normal', kernel_regularizer=l2(5e-4))(x)
    y1 = BatchNormalization()(y1)
    y1 = ReLU()(y1)
    y1 = Conv2D(num_classes, 1, kernel_initializer='he_normal', kernel_regularizer=l2(5e-4), activation='sigmoid')(y1)

    # wh header
    y2 = Conv2D(head_covn, 3, padding='same', use_bias=False, kernel_initializer='he_normal', kernel_regularizer=l2(5e-4))(x)
    y2 = BatchNormalization()(y2)
    y2 = ReLU()(y2)
    y2 = Conv2D(2, 1, kernel_initializer='he_normal', kernel_regularizer=l2(5e-4))(y2)

    # reg header
    y3 = Conv2D(head_covn, 3, padding='same', use_bias=False, kernel_initializer='he_normal', kernel_regularizer=l2(5e-4))(x)
