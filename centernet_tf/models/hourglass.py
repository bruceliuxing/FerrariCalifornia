...
def HourglassNetwork1(heads, num_stacks, cnv_dim=256, inres=(512, 512), weights='ctdet_coco',
...
def HourglassNetwork(input_layer, num_stacks, cnv_dim=256, inres=(512, 512), weights='ctdet_coco',
                     dims=[256, 384, 384, 384, 512]):
  if not (weights in {'ctdet_coco', 'hpdet_coco', None} or os.path.exists(weights)):
    raise ValueError('The `weights` argument should be either '
                     '`None` (random initialization), `ctdet_coco` '
  ...
  for i in range(num_stacks):
    prev_inter = inter
    inter = hourglass_module(None, inter, cnv_dim, i, dims)
    outputs = inter
    if i < num_stacks - 1:
      inter_ = Conv2D(cnv_dim, 1, use_bias=False, name='inter_.%d.0' % i)(prev_inter)
      inter_ = BatchNormalization(epsilon=1e-5, name='inter_.%d.1' % i)(inter_)
...
def hourglass_module(heads, bottom, cnv_dim, hgid, dims, head_dim=256):
  ...
  heads = create_heads(heads, head_dim, rf1, hgid)
  return heads, rf1
...
def create_heads(heads, head_dim, rf1, hgid):
  _heads = []
  for head in sorted(heads):
    num_channels = heads[head]
    _x = Conv2D(head_dim, 3, use_bias=True, padding='same', name=head + '.%d.0.conv' % hgid)(rf1)
